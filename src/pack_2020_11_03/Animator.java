...
public class Animator extends Thread {
    ...
    private int drawPointsCounter = 0;
...
//    }
    ...
    public void countPolynom(Polinom poly) {
        drawPointsCounter = 0;
        ...
        Point point = new Point(prev_x, prev_y);
        points.add(point);
        for (double i = -interval; i < interval; i += precision) {
            double curr_x = i;
            double curr_y = poly.getValue(i);
            points.add(new Point(curr_x, curr_y));
        }
    }

    public void drawPolynom() {
        Point firtsPoint = points.get(0);
        double prev_x = firtsPoint.getX();
        double prev_y = firtsPoint.getY();
        int border = Math.min(drawPointsCounter, points.size());
...
//            drawPencil(curr_x, curr_y);
    ...
    public void run() {
        ...
        while (true) {
            ...
            drawPoints();
            if (poly != null) {
                Point point = points.get(Math.min(drawPointsCounter, points.size() - 1));
                double x_co = point.getX();
                double y_co = point.getY();
                frameGraphics.drawImage(image, untranslateX(x_co) - (int) (point_ratio * dx), untranslateY(y_co) - dy, dx, dy, null);
                drawPolynom();
            }
